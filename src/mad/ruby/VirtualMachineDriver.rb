...
class VirtualMachineDriver < OpenNebulaDriver
    ...
    def initialize(directory, options={})
        ...
        register_action(ACTION[:off].to_sym,     method("off"))
    ...
    def off(id,drv_message)
        error = "Action not implemented by driver #{self.class}"
        send_message(ACTION[:off],RESULT[:failure],id,error)
    end
...
if __FILE__ == $0
    ...
    class TemplateDriver < VirtualMachineDriver
        ...
        def off(id, host, deploy_id, not_used)
            send_message(ACTION[:off],RESULT[:success],id)
        end
