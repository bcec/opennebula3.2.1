...
void  LifeCycleManager::backup_action(int vid)
{
    VirtualMachine *    vm;
        ...
        vm->get_lcm_state() == VirtualMachine::RUNNING)
    {
        ...
        vm->set_state(VirtualMachine::OFF_BACKUP);

        vmpool->update(vm);        

        vm->log("LCM", Log::INFO, "New VM state is OFF_BACKUP");

        //----------------------------------------------------

        vmm->trigger(VirtualMachineManager::BACKUP,vid);
    }
    else
    {
        vm->log("LCM", Log::ERROR, "backup_action, VM in a wrong state.");
...
void  LifeCycleManager::recover_action(int vid)
{
    VirtualMachine *    vm;
   ...
   if (vm->get_state() == VirtualMachine::ACTIVE &&
        vm->get_lcm_state() == VirtualMachine::RUNNING)
    {
        ...
        vm->set_state(VirtualMachine::OFF_RECOVER);

        vmpool->update(vm);
       
        vm->log("LCM", Log::INFO, "New VM state is OFF_RECOVER");

        //----------------------------------------------------

        vmm->trigger(VirtualMachineManager::RECOVER,vid);
    }
    else
    {
        ...
        vm->log("LCM", Log::ERROR, "recover_action, VM in a wrong state.");
    }
...
void  LifeCycleManager::backupdisk_action(int vid)
{
    VirtualMachine *    vm;
        ...
        vm->get_lcm_state() == VirtualMachine::RUNNING)
    {
        ...
        //----------------------------------------------------
        //                OFF_BACKUPDISK STATE
        //----------------------------------------------------

        vm->set_state(VirtualMachine::OFF_BACKUPDISK);

        vmpool->update(vm);        

        vm->log("LCM", Log::INFO, "New VM state is OFF_BACKUPDISK");

        //----------------------------------------------------

        vmm->trigger(VirtualMachineManager::BACKUPDISK,vid);
    }
    else
    {            
        vm->log("LCM", Log::ERROR, "backupdisk_action, VM in a wrong state.");
    }
...
void  LifeCycleManager::off_action(int vid)
{
    VirtualMachine *    vm;
        ...
        vm->get_lcm_state() == VirtualMachine::RUNNING)
    {
        ...
        vm->log("LCM", Log::INFO, "New state is OFFing");

        //----------------------------------------------------

        vmm->trigger(VirtualMachineManager::OFF,vid);
    }
    else
    {
        vm->log("LCM", Log::ERROR, "off_action, VM in a wrong state.");
    }
...
void  LifeCycleManager::on_action(int vid)
{
    VirtualMachine *    vm;
        ...
        vm->get_lcm_state() == VirtualMachine::OFF)
    {
        ...
        if(xmlist == "xen")
        {            
            vm->log("LCM", Log::INFO, "New VM state is ONING");
    ...
    {
        vm->log("LCM", Log::ERROR, "on_action, VM in a wrong state.");
    }
