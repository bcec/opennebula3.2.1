...
void VirtualMachineManager::trigger(Actions action, int _vid)
{
    ...
    case BACKUP:
        NebulaLog::log("DiM",Log::DEBUG,"backup1");      //added by shenxy
        aname = "BACKUP";
        break;      
...
void VirtualMachineManager::do_action(const string &action, void * arg)
{
    ...
    else if (action == "BACKUP")                //added by shenxy
    {
        NebulaLog::log("DiM",Log::DEBUG,"backup2");
        backup_action(vid);
    }
    ...
    else if (action == "BACKUPDISK")
    {
        backupdisk_action(vid);
    }
...
void VirtualMachineManager::backup_action(
    int vid)
{
    VirtualMachine *                    vm;
    const VirtualMachineManagerDriver * vmd;
    ...
    NebulaLog::log("DiM",Log::DEBUG,"backup3");
...
error_history:
    os.str("");
    os << "backup_action, VM has no history";
    goto error_common;

error_driver:
    os.str("");
    os << "backup_action, error getting driver " << vm->get_vmm_mad();
    goto error_common;
...
void VirtualMachineManager::recover_action(
    int vid)
{
    VirtualMachine *                    vm;
    const VirtualMachineManagerDriver * vmd;
...
error_history:
    os.str("");
    os << "recover_action, VM has no history";
    goto error_common;

error_driver:
    os.str("");
    os << "recover_action, error getting driver " << vm->get_vmm_mad();
    goto error_common;

error_previous_history:
    os.str("");
    os << "recover_action, VM has no previous history";
...
void VirtualMachineManager::backupdisk_action(
    int vid)
{
    VirtualMachine *                    vm;
    const VirtualMachineManagerDriver * vmd;
...
error_history:
    os.str("");
    os << "backupdisk_action, VM has no history";
    goto error_common;

error_driver:
    os.str("");
    os << "backupdisk_action, error getting driver " << vm->get_vmm_mad();
    goto error_common;
...
void VirtualMachineManager::off_action(
    int vid)
{
    VirtualMachine * vm;
    ostringstream    os;
...
error_history:
    os.str("");
    os << "off_action, VM has no history";
    goto error_common;

error_driver:
    os.str("");
    os << "off_action, error getting driver " << vm->get_vmm_mad();

error_common:
    if ( vm->get_lcm_state() == VirtualMachine::OFF ) //not in DELETE
    {
        Nebula              &ne = Nebula::instance();
        LifeCycleManager *  lcm = ne.get_lcm();
...
void VirtualMachineManager::on_action(int vid)
{
    VirtualMachine *                    vm;
    const VirtualMachineManagerDriver * vmd;
    int rc;
...
error_history:
    os.str("");
    os << "on_action, VM has no history";
    goto error_common;

error_driver:
    os.str("");
    os << "on_action, error getting driver " << vm->get_vmm_mad();
    goto error_common;

error_file:
    os.str("");
    os << "on_action, error generating deployment file: " << vm->get_deployment_file();
